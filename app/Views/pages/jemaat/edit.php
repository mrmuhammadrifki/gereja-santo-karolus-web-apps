...
<div class="container mt-4">
    ...
    <form action="<?= site_url('jemaat/update/' . $jemaat['id']) ?>" method="post">
        <?= csrf_field() ?>
        <input type="hidden" name="id_kk" value="<?= esc($jemaat['id_kk']) ?>">
        ...
        <div class="mb-3">
            ...
            <textarea name="alamat"
                class="form-control <?= isset($errors['alamat']) ? 'is-invalid' : '' ?>"><?= old('alamat', $jemaat['alamat']) ?></textarea>
        ...
        <div class="mb-3">
            ...
            <select name="rayon" class="form-select <?= isset($errors['rayon']) ? 'is-invalid' : '' ?>">
                ...
                <option value="<?= esc($r['nama_rayon']) ?>"
                    <?= old('rayon', $jemaat['rayon']) === $r['nama_rayon'] ? 'selected' : '' ?>>
                    <?= esc($r['nama_rayon']) ?>
                </option>
                <?php endforeach ?>
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="status" class="form-select <?= isset($errors['status']) ? 'is-invalid' : '' ?>">
                ...
                <option value="<?= $s ?>" <?= old('status', $jemaat['status']) === $s ? 'selected' : '' ?>><?= $s ?>
                </option>
                <?php endforeach ?>
            </select>
        ...
        <a href="<?= site_url('jemaat/detail/' . $jemaat['id_kk']) ?>" class="btn btn-secondary">Batal</a>
    </form>
</div>
