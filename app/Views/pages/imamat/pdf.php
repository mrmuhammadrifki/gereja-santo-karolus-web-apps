    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width:20%">Tempat, Tgl Tahbisan</th>
                <th>Pihak Penahbis</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($imamat as $i): ?>
            <tr>
                <td><?= $i['id'] ?></td>
                <td><?= esc($i['nama']) ?></td>
                <td>
                    <?= esc($i['tempat_tahbisan']) ?>,
                    <?= date('d-m-Y', strtotime($i['tgl_tahbisan'])) ?>
                </td>
                <td><?= esc($i['penahbis']) ?></td>
            </tr>
            <?php endforeach ?>
        </tbody>
    </table>
